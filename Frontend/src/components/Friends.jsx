...
function AllUsers() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const userRes = await axios.get(`https://spendsmart-tkm2.onrender.com/api/users`);
        const allUsers = Array.isArray(userRes.data) ? userRes.data : userRes.data.users || [];
        setUsers(allUsers);

        if (currentUserId) {
          const friendRes = await axios.get(`https://spendsmart-tkm2.onrender.com/api/users/${currentUserId}/friends`);
  ...
  const handleAddFriend = async (friendId) => {
    try {
      await axios.post(`https://spendsmart-tkm2.onrender.com/api/users/${currentUserId}/add-friend`, { friendId });
      const updatedFriends = await axios.get(`https://spendsmart-tkm2.onrender.com/api/users/${currentUserId}/friends`);
